...
// 3. CARGAR INVENTARIO (Con Botón Editar Nombre y Precio)
async function cargarInventario() {
    ...
    data.forEach(prod => {
        ...
        tr.innerHTML = `
            ...
            <td>
                <button class="btn-edit" data-id="${prod.id}" data-nombre="${prod.nombre}" data-precio="${prod.precio}" style="background:#87ceeb; color:white; border:none; padding:8px 12px; border-radius:5px; cursor:pointer; margin-right:5px;">Editar</button>
...
// 4. EVENTOS DE LA TABLA (BORRAR Y EDITAR COMPLETO)
tablaCuerpo?.addEventListener("click", async (e) => {
    ...
    // LÓGICA PARA EDITAR NOMBRE Y PRECIO
    if (e.target.classList.contains("btn-edit")) {
        const nombreActual = e.target.getAttribute("data-nombre");
        const precioActual = e.target.getAttribute("data-precio");

        // Paso 1: Preguntar el nuevo nombre
        const nuevoNombre = prompt("Editar nombre del producto:", nombreActual);
        if (nuevoNombre === null) return; // Si cancela, salir

        // Paso 2: Preguntar el nuevo precio
        const nuevoPrecio = prompt("Editar precio del producto:", precioActual);
        if (nuevoPrecio === null) return; // Si cancela, salir
